...
if (!empty($where))$where.=' AND '.$_POST['where'];
...
// обработка вывода результата
 ...
 /////////////////////////////////////////////    
if ($dat[$k]['tip']=='img'){if ($v[$k]!=''){
    
$raz=getimagesize(SITE_PATH.$dat[$k]['path'].$v[$k]);

if ($raz[0]>$raz[1])$v[$k]='<img src="'.$dat[$k]['path'].$v[$k].'" width="60px"/>';
else $v[$k]='<img src="'.$dat[$k]['path'].$v[$k].'" height="60px"/>';
...
if ($dat[$k]['tip']=='img_m'){if ($v[$k]!=''){
$m_im=$db->select("SELECT * FROM ".DB_PREFIX.$dat[$k]['tabl']." WHERE id in($v[$k])");  
$v[$k]='';    
foreach ($m_im as $kkk => $vvv) {    
$raz=getimagesize(SITE_PATH.$dat[$k]['path'].$vvv['img']);
if ($raz[0]>$raz[1])$v[$k].='<img src="'.$dat[$k]['path'].$vvv['img'].'" width="60px"/> ';
else $v[$k].='<img src="'.$dat[$k]['path'].$vvv['img'].'" height="60px"/> ';
}
...
if ($dat[$k]['tip']=='sort'){$v['up_down']='<span class="up" onclick="sort'.$tabl.'(\'up\',\''.$v[sort].'\')"  ></span> <span class="down" onclick="sort'.$tabl.'(\'down\',\''.$v[sort].'\')"></span>';}    
 ...
 $v['f_edit']='<a href="javascript:void(0);" onclick="edit'.$tabl.' ('.$v[id].')">Редактировать</a>';
 $v['f_del']='<a href="javascript:void(0);" onclick="del'.$tabl.' ('.$v[id].')" >Удалить</a>';
