...
$sql =$db->select('SELECT * FROM '.DB_PREFIX.'news WHERE en=1  ORDER BY `data` desc LIMIT '.$pagin[1]);
...
<li>
                                    ...
                                    <h3><a href="'.$site_url.'/news/'.$v['id'].'">'.$v['zag_ru'].'</a></h3>
                                    <p class="news_text">'.str_smoll($v['text_'.$site_language],300).'</p>
                                    <a href="'.$site_url.'/news/'.$v['id'].'">Подробнее</a>
                                </li>   
';
