...
<script type="text/javascript">
   ...
   function showRequest<?=$GLOBALS['ta_tabl']?> (){
    $("#modload_<?=$GLOBALS['ta_tabl']?>" ).html('<center><img src="/admin/css/images/load.gif"/></center>');  
   }           
  //после отправки формы закрывает окно
  // и обновляет таблицу
   function showResponse<?=$GLOBALS['ta_tabl']?> (){
   close_win( "#modload_<?=$GLOBALS['ta_tabl']?>" ); 
   ...
   function go<?=$GLOBALS['ta_tabl']?> (com, grid) {
	...
	$.each( obj ,function(){id=id+$(this).attr('id').substring(3)+',';});
     ...
     if (com=="Добавить"){
         ...
         data: {tabl:'<?=$GLOBALS['ta_tabl']?>', dat: '<?=$GLOBALS['ta_dat']?>', dop_wh: '<?=$GLOBALS['ta_dw']?>'}
          ...
          <button onclick="close_win (\'#modload_<?=$GLOBALS['ta_tabl']?>\');$(\'#modload_<?=$GLOBALS['ta_tabl']?>\').html(\'\');">Отменить</button>');
    
          $("#modload_<?=$GLOBALS['ta_tabl']?> button").button();
          
          open_win ('#modload_<?=$GLOBALS['ta_tabl']?>','Добавить - <?=$GLOBALS['ta_title']?>')
         ...
         $("#modload_<?=$GLOBALS['ta_tabl']?> form").validationEngine();
          tiniini();
           ...
           $(".daty").datepicker({ dateFormat: "yy-mm-dd" });
    ...
    if (com=="Редактировать"){
     if (col!=1){alert("Выберите одну запись!!!");}
     else {
                $.ajax({
                type: "POST",
          ...
          open_win ('#modload_<?=$GLOBALS['ta_tabl']?>','Редактировать - <?=$GLOBALS['ta_title']?>');      
   ...
   function del<?=$GLOBALS['ta_tabl']?> (id){
   if (confirm("Удалить?") ){
         $.ajax({
         type: "POST",
         url: "/core/admin/del_bd.php",
   ...
   function edit<?=$GLOBALS['ta_tabl']?> (id){
