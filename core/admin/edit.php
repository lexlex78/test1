...
// вывод инпутов
$input_all='<form id="form_'.$_POST['tabl'].'" name="form_'.$_POST['tabl'].'" action="/core/admin/edit_bd.php" method="POST" enctype="multipart/form-data" onSubmit="return false">
<div class="t_editor" >
...
if ($v['tip']=='color'){
    ...
    $val='<input '.$valid.' readonly="readonly" type="text" name="'.$k.'" value="'.$sel[$k].'" />';}

if ($v['tip']=='img_m'){$valid='class="validate[custom[img]]"';
$val='<div style="max-width: 680px;">';
if ($sel[$k]!=""){
$pv=$db->select("SELECT * FROM ".DB_PREFIX.$v['tabl']." WHERE id in ($sel[$k])");
foreach ($pv as $key => $value) {
$val.='<div style="display: inline-block;"> <img src="'.$dat[$k]['path'].$value['img'].'" width="150px"/><br><center><a href="javascript:void(0)" onclick="del_m_f (this,\''.$value['id'].'\')">удалить</a></center><br></div> ';    
}
$val.='</div>';
}

    $val.='<input type="hidden" name="'.$k.'_del" /><br><hr><input '.$valid.' multiple="true" type="file" name="'.$k.'[]" />';
$name.='<br>';
...
if ($v['tip']=='img'){$valid='class="validate[custom[img]]"';
