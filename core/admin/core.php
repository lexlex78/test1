...
function tab_admin($t,$title,$d,$d1,$dw){
 //  инициализация переменных 
 global $ta_title,$ta_colModel,$ta_searchitems,$ta_sortname,$ta_tabl,$ta_dat,$ta_sortorder,$ta_where,$ta_dat1,$t_dop_but,$t_dop_but_sc,$ta_dw;
$ta_where=$d1['where'];
$ta_title=$title;$ta_colModel='';$ta_searchitems='';
$ta_sortname='id';$ta_tabl=$t;$ta_sortorder='desc';
$ta_dat= serialize($d);
$ta_dat1=$d1;
$ta_dw=$dw;
...
$width=70;
if ($v['tip']=='img')$width=90;
if ($v['tip']=='img_all')$width=100;
...
if ($v['tip']=='img_all' ||$v['tip']=='img' ||  $v['tip']=='sort'){}
...
if ($d1['edit']!='0') $ta_colModel.="{display: '&nbsp;', name : 'f_edit', sortable : false, width : 80, align: 'center'},";
if ($d1['del']!='0') $ta_colModel.="{display: '&nbsp;', name : 'f_del', sortable : false, width : 50, align: 'center'},";
...
$dost_in=  explode(',',$dantree[$v]['access']);   
 ...
 if ($dantree[$v]['cild'])
 {
     ...
     $ret.='<li '.$active.'><a>'.$dantree[$v]['name'].'</a>'.vievtree_top ($dantree[$v]['id'],$name,$tabl,$num).'</li>';}
...
$ret.='<ul '.$fol.'>';
foreach ($dantree[$a]['cild'] as $v) {
    $actti=''; if ($_GET['tree_'.$num]==$dantree[$v]['id'])$actti=' style="background: #000;color: #fff;padding: 5px" ';
 if ($dantree[$v]['cild'])
 {$fol='class="closed"';if ($dantree[$v]['tree_num']==1)$fol='';
     $ret.='<li '.$fol.'><span class="folder" '.$actti.'>'.$dantree[$v][$name].'</span>'.vievtree ($dantree[$v]['id'],$name,$tabl,$num).'</li>';}
 else $ret.='<li><a '.$actti.' href="/admin/?r='.$_GET['r'].'&tree_'.$num.'='.$dantree[$v]['id'].'">'.$dantree[$v][$name].'</a></li>';
...
// рекурсивная функцти для вывода дерева где все ветки являются ссылками

function vievtree_a ($a,$name,$tabl,$num){
$ret='';
global $dantree;
$fol='';if ($a==0)$fol='id="'.$tabl.'_'.$num.'"';
$ret.='<ul '.$fol.'>';
foreach ($dantree[$a]['cild'] as $v) {
 ...
 if ($dantree[$v]['cild'])
     {$fol='class="closed"';if ($dantree[$v]['tree_num']==1)$fol='';
     $ret.='<li '.$fol.'><a '.$actti.' href="/admin/?r='.$_GET['r'].'&tree_'.$num.'='.$dantree[$v]['id'].'">'.$dantree[$v][$name].'</a>'.vievtree_a ($dantree[$v]['id'],$name,$tabl,$num).'</li>';}
 else $ret.='<li><a '.$actti.' href="/admin/?r='.$_GET['r'].'&tree_'.$num.'='.$dantree[$v]['id'].'">'.$dantree[$v][$name].'</a></li>';
...
//функцти  вывода дерева где все ветки являются ссылками
// $tabl таблица
// $d данные
function tree_db_a($tabl,$d,$num)
{ global $dantree,$db;
  $dantree="";
  $dantree=$db->select_id("SELECT * FROM ".DB_PREFIX.$tabl." ORDER BY ".$d['sort']." ASC");
     //1 я итерация указываем каждому родителю ребенка
    ...
    $tree=vievtree_a (0,$d['name'],$tabl,$num);
    return $tree;
...
////////////////////////////////////////////////////////////////////////////
// вывод дерево где все ветки являются ссылками
//$t - таблица
//$d - масив с типами полей
//$title - заголовок


function tree_admin_allhref($t,$title,$d,$num,$z_where){
global $tree_title,$tree_dan,$tree_tab,$tree_num;
