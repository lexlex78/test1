...
// выход $soput;
 ...
 $soput='';
 if ($dat['soput']){
  $soput.='<h3 class="title">Сопутствующие товары</h3>
                            <div class="clearfix related_items_slider">
                                <ul id="3items_slider" class="jcarousel-skin-tango display_table clearfix">';   
 $zz=$db->select('SELECT p.* , i.img  , e.name as f_en FROM '.DB_PREFIX.'shop_produkt  as p LEFT JOIN '.DB_PREFIX.'shop_produkt_img as i ON  p.id=i.pid  LEFT JOIN '.DB_PREFIX.'f_en as e  ON p.f_en=e.id  WHERE p.en=1 and p.id IN ('.$dat['soput'].')  GROUP BY p.id');
